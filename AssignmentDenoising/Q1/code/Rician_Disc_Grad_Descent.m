function running_sol = Rician_Disc_Grad_Descent(imageNoisy,params,num_iter,show_plot)
%RICIAN_DISC_GRAD_DESCENT 
...
running_obj_val = Rician_Disc_Obj(running_sol,imageNoisy,alpha,gamma);

for i = 1:num_iter
    
    grad_matrix = Rician_Disc_Grad(running_sol, imageNoisy, alpha,gamma);
    potential_sol = running_sol - running_step.*grad_matrix;
    new_obj_val = Rician_Disc_Obj(potential_sol,imageNoisy,alpha,gamma);
...
if show_plot == 1
   figure;
    ...
    title('Adaptive-Discontinuity penalty'); 
    xlabel('iterations'); 
    ylabel('Objective function');
    save('../results/obj_disc.mat',"monitor_loss");
end
